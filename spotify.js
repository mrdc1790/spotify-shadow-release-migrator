...
export function parseSpotifyInput(input) {
    ...
    throw new Error('Enter a Spotify track or album URL, URI, or 22-character ID.');
}
export function parseTrackId(input) {
    const parsed = parseSpotifyInput(input);
    if (parsed.type === 'album') throw new Error('Expected a track, but received an album.');
    return parsed.id;
}
...
export function buildAlbumReplacements(sourceTracks, destinationTracks) {
    ...
    const byPosition = new Map(
        destinationTracks.map((track) => [`${track.disc_number}:${track.track_number}`, track]),
    );
    const replacements = sourceTracks.map((source) => {
        const destination = byPosition.get(`${source.disc_number}:${source.track_number}`);
        if (!destination)
            throw new Error(
    ...
    if (new Set(replacements.map((item) => item.destinationUri)).size !== replacements.length)
...
export function findTargets(playlists, contents, replacements) {
    ...
    return playlists.flatMap((playlist) => {
        ...
        const uris = rows.map(itemUri),
            count = (values) =>
                Object.fromEntries(
                    [...values].map((uri) => [uri, uris.filter((value) => value === uri).length]),
                ),
        ...
        return [
            {
                ...
                occurrences,
                sourceUris,
                destinationUrisPresent: [...new Set(uris.filter((uri) => destinations.has(uri)))],
                sourceCounts: count(sourceUris),
                destinationCounts: count([...new Set(uris.filter((uri) => destinations.has(uri)))]),
            },
        ];
    });
}
export function findLibraryTarget(replacements, membership) {
    const sourceUris = replacements
        ...
        destinationUrisPresent = [
            ...new Set(
                replacements.map((r) => r.destinationUri).filter((uri) => membership[uri] === true),
            ),
        ];
    return {
...
async function chunks(items, size, action) {
    for (let i = 0; i < items.length; i += size) await action(items.slice(i, i + size));
}
...
function sameCounts(expected, actual) {
    return Object.entries(expected).every(([uri, count]) => (actual[uri] || 0) === count);
}
export async function runSafeMigration(targets, replacements, api, inspectPlaylist) {
    const bySource = new Map(replacements.map((r) => [r.sourceUri, r.destinationUri]));
    ...
    if (results.some((result) => result.error)) return { phase: 'preflight-failed', results };
    for (let i = 0; i < results.length; i++) {
        const result = results[i],
            target = targets[i],
            needed = target.sourceUris
                .flatMap((uri) => Array(target.sourceCounts?.[uri] || 1).fill(bySource.get(uri)))
                .filter(Boolean);
        try {
    ...
    if (results.some((result) => !result.added)) return { phase: 'add-failed', results };
    for (let i = 0; i < results.length; i++) {
        const result = results[i],
            target = targets[i];
        try {
            ...
            for (const [source, count] of Object.entries(
                target.sourceCounts || Object.fromEntries(target.sourceUris.map((uri) => [uri, 1])),
            )) {
                const destination = bySource.get(source);
                expectedDest[destination] = (expectedDest[destination] || 0) + count;
            }
            if (!sameCounts(expectedDest, actual) || !sameCounts(target.sourceCounts || {}, actual))
                throw new Error(
    ...
    return {
        phase: results.every((result) => result.removed) ? 'complete' : 'remove-partial',
        results,
    };
}
export async function prepareLibraryMigration(target, replacements, api, inspectLibrary) {
    if (!target?.affected)
        return {
            phase: 'not-affected',
            sourceUris: [],
    ...
    if (Object.entries(target.expectedMembership).some(([uri, saved]) => current[uri] !== saved))
        return {
            phase: 'preflight-failed',
            error: 'Liked Songs changed since preview; rebuild the preview before migrating.',
        };
    const bySource = new Map(replacements.map((r) => [r.sourceUri, r.destinationUri])),
        destinationUris = [
            ...new Set(target.sourceUris.map((uri) => bySource.get(uri)).filter(Boolean)),
        ],
    ...
    try {
        ...
        const verified = await inspectLibrary([...target.sourceUris, ...destinationUris]);
        if (
            target.sourceUris.some((uri) => !verified[uri]) ||
            destinationUris.some((uri) => !verified[uri])
        )
            throw new Error('Liked Songs replacement verification failed; old likes were kept.');
        return {
            phase: 'prepared',
...
export async function runSafeSwap(targets, sourceUri, destinationUri, api) {
    return runSafeMigration(
        targets.map((t) => ({
            ...
            destinationUrisPresent:
                t.destinationUrisPresent || (t.destinationPresent ? [destinationUri] : []),
        })),
