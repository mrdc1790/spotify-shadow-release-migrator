...
test("adds and verifies a replacement before removing the old like", async () => {
  const saved = { a: true, b: false },
    calls = [],
    inspect = async (uris) =>
      Object.fromEntries(uris.map((uri) => [uri, Boolean(saved[uri])])),
