...
async function tokenRequest(body) {
  ...
  if (!response.ok)
    throw new Error(
      "Spotify login failed. Check the Client ID and redirect URI.",
    );
...
async function finish(code, returned) {
  ...
  if (!verifier || returned !== expected)
    throw new Error("Invalid login state. Connect again.");
...
async function api(path, options = {}) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    await new Promise((r) =>
      setTimeout(
        r,
        Math.min(Number(response.headers.get("Retry-After") || 1), 10) * 1000,
      ),
    );
  }
  if (!response.ok) {
    ...
    const error = new Error(
      detail || `Spotify request failed (${response.status}).`,
    );
...
async function scan() {
  try {
    ...
    const allUris = [
        ...new Set(
          resolved.replacements.flatMap((r) => [r.sourceUri, r.destinationUri]),
        ),
      ],
    ...
    for (let i = 0; i < playlists.length; i++) {
      message(
        `Scanning playlist ${i + 1} of ${playlists.length}: ${playlists[i].name}`,
      );
      try {
        contents.set(
          playlists[i].id,
        ...
        state.skipped.push({
          ...
          category:
            error.status === 403 ? "not owner/collaborator" : "read failed",
        });
      }
    }
...
async function inspectPlaylist(id) {
  ...
  if (!playlist.snapshot_id)
    throw new Error(
      "Spotify did not return a playlist snapshot; source tracks were kept.",
    );
...
async function init() {
  ...
  if (
    !params.get("code") &&
    localStorage.getItem("spotify_scope_version") !== scopeVersion
  ) {
  ...
  try {
    if (params.get("error"))
      throw new Error(`Login cancelled: ${params.get("error")}`);
